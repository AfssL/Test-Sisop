...
ns1 IN  A   192.194.3.10  ; IP Renoir
ns2 IN  A   192.194.3.20  ; IP Verso
@   IN  A   192.194.2.10  ; IP Lune
...
ns1 IN  A   192.194.3.10  
ns2 IN  A   192.194.3.20  
@   IN  A   192.194.2.10  
...
nslookup 192.194.2.30 192.194.3.10
nslookup 192.194.2.30 192.194.3.20
...
ping lune33.com
# Harusnya TETAP BISA (dijawab Verso, 192.194.2.10)

ping exp.sciel33.com
# Harusnya TETAP BISA (dijawab Verso, 192.194.2.20)

ping gustave33.com
# Harusnya TETAP BISA (dijawab Verso, 192.194.2.30)
...
upstream info_servers {
    server 192.194.2.10:8000; # Lune Info
    server 192.194.2.20:8100; # Sciel Info
    server 192.194.2.30:8200; # Gustave Info
}
...
server {
    ...
    location /profil_lune {
        proxy_pass http://192.194.2.10:80/;
        proxy_set_header Host $host;
    }

    location /profil_sciel {
        proxy_pass http://192.194.2.20:80/;
        proxy_set_header Host $host;
    }

    location /profil_gustave {
        proxy_pass http://192.194.2.30:8080/;
        proxy_set_header Host $host;
    }
...
LUNE	"cat /tmp/access.log"	Harus ada 1 baris log baru dari IP Alicia (192.194.4.10).
SCIEL	"cat /tmp/access.log"	Harus ada 1 baris log baru dari IP Alicia (192.194.4.10).
GUSTAVE	"cat /tmp/access.log"	Harus ada 1 baris log baru dari IP Alicia (192.194.4.10).